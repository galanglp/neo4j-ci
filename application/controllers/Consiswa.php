...
class Consiswa extends CI_Controller {
	...
	{
		$this->mylib->load_view('Siswa','siswa');
	}
	...
	public function save(){
		if ($this->input->post('method') == "save") {
            ...
            $this->siswa_model->saveStatus(array('status' => "Lolos"));
	...
	public function ajax_list(){
		...
		$query = $this->siswa_model->read(array('Lolos','Alumni'));
		$data = array();
		$no = 0;
		foreach ($query as $key => $value) {
			...
			$row[] = ($value['status']=="Lolos") ? '<span class="label label-success">Siswa</span>' : '<span class="label label-info">'.$value['status'].'</span>';
		...
		$output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->siswa_model->count_all(array('Lolos','Alumni')),
                        "recordsFiltered" => $this->siswa_model->count_filtered(array('Lolos','Alumni')),
                        "data" => $data,
                );
